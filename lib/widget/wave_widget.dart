...
class _WaveWidgetState extends State<WaveWidget> with TickerProviderStateMixin {
  ...
  late Map<Animation<double>, AnimationController> valueList;
  ...
  void initState() {
    ...
    widget.controller?.addListener(() {
      if (widget.controller?.isAnimation == true) {
        _resumeAnimation();
      } else {
        _pauseAnimation();
      }
...
class _CustomWavePainter extends CustomPainter {
  ...
  _setPaths(double viewCenterY, Size size, Canvas canvas) {
    Layer _layer = Layer(
      ...
      gradient: gradient,
      blur: blur,
      amplitude: (-1.6 + 0.8) * (waveAmplitude ?? 0),
      phase: (wavePhaseValue?.value ?? 0) * 2 + 30,
    );
    ...
    if (_layer.gradient != null) {
      var rect = Offset.zero &
          Size(
              size.width, size.height - viewCenterY * (heightPercentange ?? 0));
  ...
  void paint(Canvas canvas, Size size) {
    double viewCenterY = size.height * ((heightPercentange ?? 0) + 0.1);
  ...
  double _getSinY(
    ...
    return (sin(_tempA * (waveFrequency ?? 0) * ((currentposition ?? 0) + 1) +
        (startradius ?? 0) * _tempB));
  }
}
