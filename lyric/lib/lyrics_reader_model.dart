...
class LyricsReaderModel {
  ...
  double computeScroll(int toLine, int playLine, LyricUI ui) {
    ...
    //Need special handling for the first line offset upwards
  ...
  double firstCenterOffset(int playIndex, LyricUI lyricUI) {
    return LyricHelper.centerOffset(lyrics.length > 0 ? lyrics.first : null,
        playIndex == 0, lyricUI, playIndex);
  }

  double lastCenterOffset(int playIndex, LyricUI lyricUI) {
    return LyricHelper.centerOffset(lyrics.length > 0 ? lyrics.last : null,
        playIndex == lyrics.length - 1, lyricUI, playIndex);
  }
}
