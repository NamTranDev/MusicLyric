...
class LyricsReaderPaint extends ChangeNotifier implements CustomPainter {
  ...
  bool checkOffset(double? offset) {
    ...
    LyricsLog.logD("Cancel Offset Offset Offset: $maxOffset Target Offset: $offset Current: $_lyricOffset ");
    return false;
  }
  ...
  void calculateTotalHeight() {
    ...
    if (cachePlayingIndex != playingIndex) {
      ...
      //Maximum offset does not include the last row
  ...
  ///give the external C bit position
  var centerY = 0.0;
    ...
    for (var i = 0; i < lyrics.length; i++) {
      ...
      if (centerY > drawOffset && centerY < nextOffset) {
        if (i != centerLyricIndex) {
          centerLyricIndex = i;
          LyricsLog.logD(
              "drawOffset:$drawOffset next:$nextOffset center:$centerY  Current line is: $i Text:${element.mainText} ");
        }
      }
  ...
  double _drawOtherLyricLine(Canvas canvas, double drawOffsetY,
    ...
    //First line without line spacing
    if (lineIndex != 0) {
    ...
    if (element.hasExt) {
      //Padding is only added when there are main lyrics
      if (element.hasMain) {
  ...
  void drawHighlight(LyricsLineModel? model, Canvas canvas, TextPainter? painter,
      Offset offset) {
    if (model == null || model.hasMain == false) return;
  ...
  ///draw text and return line height
  ...
  double drawText(Canvas canvas, TextPainter? paint, double offsetY,
    ...
    //paint It is theoretically impossible to be empty, and an error is expected
  ...
  ///Get the horizontal coordinates of the line drawn
